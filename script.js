...
async function loadPosts() {
    try {
        ...
        // Если указан конкретный пост, скроллим к нему
        if (postId) {
            setTimeout(() => {
    ...
    postDiv.innerHTML = `
        <div class="post-header">
            ...
            ${post.userId === currentUserId ? `
                <i class="fas fa-edit" onclick="openEditPost('${post.id}', '${post.title}', '${post.content}')" style="color:#3b82f6; cursor:pointer; margin-right:10px;"></i>
...
async function addComment() {
    ...
    try {
        const response = await fetch(`/api/comment/${currentPostId}`, {
            method: 'POST',
...
async function subscribeToChannel(channelId) {
    try {
        const response = await fetch(`/api/subscribe-channel/${channelId}`, {
            method: 'POST'
        });
...
function updateSelectedUsers() {
    ...
    selectedUsers.forEach(userId => {
        fetch(`/api/search?q=${userId}&type=users`)
            .then(res => res.json())
            .then(data => {
...
async function addToGroup(chatId, userId) {
    try {
        ...
        const updateResponse = await fetch(`/api/chat/${chatId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
